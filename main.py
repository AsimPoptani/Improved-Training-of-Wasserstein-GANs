...
class Generator(L.LightningModule):
    def __init__(self, depth=1, noise_size=(1,1), noise_channels=100):
        ...
        self.model=torch.nn.Sequential(
            ...
            torch.nn.Conv2d(in_channels=self.fire_power*8, out_channels=self.fire_power*8, kernel_size=3, stride=1,padding=1),
            torch.nn.LeakyReLU(),
            torch.nn.LayerNorm([self.fire_power*8,4,4]),
            ...
            torch.nn.Conv2d(in_channels=self.fire_power*4, out_channels=self.fire_power*4, kernel_size=3, stride=1,padding=1),
            torch.nn.LeakyReLU(),
            torch.nn.LayerNorm([self.fire_power*4,8,8]),
            ...
            torch.nn.Conv2d(in_channels=self.fire_power*2, out_channels=self.fire_power*2, kernel_size=3, stride=1,padding=1),
            torch.nn.LeakyReLU(),
            torch.nn.LayerNorm([self.fire_power*2,16,16]),
            ...
            torch.nn.Conv2d(in_channels=self.fire_power, out_channels=self.fire_power, kernel_size=3, stride=1,padding=1),
            torch.nn.LeakyReLU(),
            torch.nn.LayerNorm([self.fire_power,32,32]),
...
class ImprovedWassersteinGAN(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        #     # Calculate the loss
            ...
            # dis_loss = torch.mean(fake_scores) - torch.mean(scores)
        #
        #     # Update the discriminator
            dis_optimizer.zero_grad()
            dis_loss.backward()
            dis_optimizer.step()
            self.logger.experiment.add_scalars('main_tag',{"avg_true_score":scores.mean(),"avg_fake_score":fake_scores.mean()},self.counter)
...
if __name__ == "__main__":
    ...
    gen=torch.compile(Generator(depth=2,noise_channels=3072))
    dis=torch.compile(Discriminator(depth=3,image_size=(32,32)))
    trainer.fit(ImprovedWassersteinGAN(gen,dis), cifar100)
