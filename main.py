...
class Generator(L.LightningModule):
    def __init__(self, depth=1, noise_size=(1,1), noise_channels=100):
        ...
        self.model=torch.nn.Sequential(
            ...
            torch.nn.Conv2d(in_channels=self.fire_power*8, out_channels=self.fire_power*8, kernel_size=3, stride=1),
            torch.nn.LeakyReLU(),
            torch.nn.LayerNorm([self.fire_power*8,4,4]),
            ...
            torch.nn.Conv2d(in_channels=self.fire_power*4, out_channels=self.fire_power*4, kernel_size=3, stride=1),
            torch.nn.LeakyReLU(),
            torch.nn.LayerNorm([self.fire_power*4,8,8]),
            ...
            torch.nn.Conv2d(in_channels=self.fire_power*2, out_channels=self.fire_power*2, kernel_size=3, stride=1),
            torch.nn.LeakyReLU(),
            torch.nn.LayerNorm([self.fire_power*2,16,16]),
            ...
            torch.nn.Conv2d(in_channels=self.fire_power, out_channels=self.fire_power, kernel_size=3, stride=1),
            torch.nn.LeakyReLU(),
            torch.nn.LayerNorm([self.fire_power,32,32]),
